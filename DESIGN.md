...
- **The parity gallery is the brand-range regression surface.** `libs/table/.storybook/parity/` (the "Atoms" Storybook section: 33 Material surfaces, 45 stories) renders every Angular Material surface under the brand × mode toolbar. Any contract change, brand addition, or Material token override must keep all 12 brands × 2 modes presentable there — the gallery is where "does the engine still disappear behind the brand?" gets answered.
