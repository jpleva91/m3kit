...
| Optional adapters you create (`google-maps`, `maplibre`, `xlsx`, `pdf`, telemetry sinks, persistence adapters) | **Create in your workspace, not in baseline core** | Adapter code translates your vendor/runtime policy to m3kit contracts. Keep API keys, billing, SDK lifecycle, browser downloads, and backend job polling out of `core` and out of the copyable baseline unless your own ADR accepts them. |
| `apps/demo-reporting` | **Delete / do not copy** | Demo-only. It exists as living documentation of how the libs wire together, including no-key map/static reporting examples. Everything reusable lives in the libs; the app is disposable by design. |
