...
| `libs/core` (`@m3kit/core`) | UI-free contracts: table/column definitions, query/filter/sort/pagination models, datasource interfaces, typed report errors, load states, versioned query serialization, column view state, saved views, export helpers, temporal policy, and telemetry contracts. No Material/CDK, no internal dependencies. |
| `libs/table` (`@m3kit/table`) | Material/CDK table layer: `m3k-data-table` (including `columnState`), `m3k-table-filter-bar`, `m3k-page-toolbar`, `m3k-tree`. May depend on core only. |
...
| `libs/state` (`@m3kit/state`) | NgRx SignalStore features: `withDataQuery` (loadState/stale/error taxonomy and telemetry emission), `withSelection`, theme store, and the `REPORT_TELEMETRY_REPORTER` no-op/default injection token. Depends on core only. |
