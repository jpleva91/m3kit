...
- [x] T020 [US3] Upgrade the invoices report `apps/demo-reporting/src/app/reports/reports.component.{ts,html,scss}` (after T017, T018, T019): wire `report-url-state` (restore on load, sync on change), saved-view picker on the page-toolbar action slot (MatMenu, precedent: app brand switcher) applying query + `columnState` together and updating the URL, export menu (CSV/JSON × current page/all filtered rows) building `ExportRequest` from the live store query + resolved visible columns through `createExportResult` + `export-download`, `markStale`-driven `m3k-banner` + refresh affordance, `reportId` set on the store for telemetry. Token-only styling for anything visual; synthetic data only. Done: `npx nx lint demo-reporting` green
- [x] T021 [US3] Update invoices page coverage `apps/demo-reporting/src/app/reports/reports.component.spec.ts` + `reports.component.stories.ts` (after T020): spec asserts URL round-trip (SC-002), saved-view application updates rows + columns + URL together, export content matches the live query's rows for both formats and scopes (SC-003, no network), telemetry events observed via a test reporter; story keeps the `withDisabledInitialNavigation` router pattern. Done: `npx nx test demo-reporting` green
- [x] T022 [P] [US3] Reuse the URL-state helper on the customers report `apps/demo-reporting/src/app/reports/customers-report.component.{ts,spec.ts}` (after T017): decode-on-init + sync-on-change only (no saved views/export — reuse proof, FR-016). Done: customers spec covers the round-trip; serve-and-walk confirms both pages restore from copied URLs
...
- [x] T023 [P] Fill `docs/REPORTING_FOUNDATION.md` (skeleton from T002): server-side query contract walkthrough (`DataQuery`/`DataPage` ↔ backend request/response translation example, connect-free fetch semantics, `totalCount`), state-taxonomy → `@m3kit/feedback` mapping table, saved-view + URL-state pattern (incl. which demo files are app policy to replace), export baseline-vs-adapter boundary table, temporal conventions (UTC instants, half-open ranges), telemetry redaction rule
- [x] T024 [P] Update the remaining docs to stay truthful: AGENTS.md component/contract inventory (core gains the foundation modules; table's `columnState`; state's `loadState`/telemetry token) and HOW-TO additions if warranted; README repo map if it lists core modules; finalize the T001 ADR against what actually shipped; confirm `docs/BOUNDARY_LOG.md` contains an entry for every external consultation made during the feature (target: none) — logged at consultation time, never retroactively
- [x] T025 Full gate (strictly last): `npx nx run-many -t lint test build && ELECTRON_EXTRA_LAUNCH_ARGS=--no-sandbox npx nx run-many -t component-test && npx nx run m3kit-table:build-storybook` — all green, taxonomy + column-state stories visible in the Storybook build, core import audit re-run (`grep -rE "@angular|material|cdk" libs/core/src` empty), `package.json` diff-free; record completion evidence at the foot of this file

### T025 Completion Evidence

- `npx nx run-many -t lint test build` passed for 12 projects. Existing bundle/style budget warnings remain unchanged.
- `ELECTRON_EXTRA_LAUNCH_ARGS=--no-sandbox npx nx run-many -t component-test --skip-nx-cache` hit transient Cypress/Xvfb/Electron flakes in parallel, then the failed targets passed individually:
  - `npx nx run m3kit-charts:component-test --skip-nx-cache` passed.
  - `npx nx run m3kit-shell:component-test --skip-nx-cache` passed.
  - `npx nx run m3kit-dashboard:component-test --skip-nx-cache` passed.
  - The aggregate run also passed the remaining component-test projects (`m3kit-feedback`, `m3kit-forms`, `m3kit-table`).
- `npx nx run m3kit-table:build-storybook` passed; built Storybook output includes the report load-state taxonomy and column-state/store-driven stories.
- Core import audit passed: `grep -rE "@angular|material|cdk" libs/core/src` returned no matches.
