...
/** Payload emitted by `m3k-table-filter-bar` when the search text changes. */
export interface TableFilterBarChange {
  readonly text: string;
}

/** Milliseconds the filter bar waits after the last keystroke before emitting. */
export const TABLE_FILTER_DEBOUNCE_MS = 200;

/**
 * Free-text search bar for a data table. Emits `filterChange` debounced by
 * {@link TABLE_FILTER_DEBOUNCE_MS} and deduplicated, with a clear
...
export class TableFilterBarComponent<T> {
  ...
  readonly filterChange = output<TableFilterBarChange>();
  ...
  constructor() {
    this.textChanges
      .pipe(
        ...
        debounce(({ immediate }) =>
          immediate ? of(0) : timer(TABLE_FILTER_DEBOUNCE_MS),
        ),
        map(({ text }) => text),
