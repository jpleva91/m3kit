...
@Component({
  ...
  template: `<m3k-breadcrumbs [items]="items" />`,
})
class HostComponent {
...
describe('BreadcrumbsComponent', () => {
  ...
  it('hides the separators from assistive technology', () => {
    const separators = Array.from(
      element().querySelectorAll('.m3k-breadcrumbs__separator'),
    );
  ...
  it('renders an intermediate item without a path as plain text', () => {
    ...
    expect(
      element().querySelector('.m3k-breadcrumbs__text')?.textContent?.trim(),
    ).toBe('Reports');
    expect(
  ...
  it('renders a single item as the current page only — no links, no separators', () => {
    ...
    expect(element().querySelectorAll('a')).toHaveLength(0);
    expect(element().querySelectorAll('.m3k-breadcrumbs__separator')).toHaveLength(0);
    expect(
      element().querySelector('[aria-current="page"]')?.textContent?.trim(),
    ).toBe('Dashboard');
  });
