...
describe(BarChartComponent.name, () => {
  it('renders token-colored bars per category and series', () => {
    mountBarChart();
    cy.get('svg.m3k-bar-chart').should('have.attr', 'role', 'img');
    cy.get('.m3k-bar-chart__bar').should('have.length', 4);
    cy.get('.m3k-bar-chart__bar[data-series="0"]').should(
      'have.attr',
      'fill',
      'var(--app-chart-1)',
    );
    cy.get('.m3k-bar-chart__bar[data-series="1"]').should(
      'have.attr',
      'fill',
      'var(--app-chart-2)',
    );
    cy.get('.m3k-bar-chart__tick--category').should('have.length', 2).first().should('contain.text', 'Q1');
  });
  ...
  it('stacks segments whose heights sum to the scaled category total', () => {
    ...
    cy.get('.m3k-bar-chart__bar[data-category="0"]').then(($bars) => {
  ...
  it('swaps axes in horizontal mode', () => {
    ...
    cy.get('.m3k-bar-chart__bar[data-category="0"][data-series="0"]')
      .invoke('attr', 'x')
      .should('eq', '88');
    cy.get('.m3k-bar-chart__bar').each(($bar) => {
  ...
  it('renders an empty accessible SVG without data', () => {
    ...
    cy.get('svg.m3k-bar-chart').should('have.attr', 'role', 'img');
    cy.get('.m3k-bar-chart__bar').should('not.exist');
  });

  it('sizes the viewBox to the measured host width (1 unit = 1 CSS px)', () => {
    mountBarChart();
    cy.get('svg.m3k-bar-chart').should(($svg) => {
