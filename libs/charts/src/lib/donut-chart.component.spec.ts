...
describe('DonutChartComponent', () => {
  ...
  const slices = (): readonly Element[] =>
    [...element().querySelectorAll('.m3k-donut-chart__slice')];
  ...
  it('renders an accessible empty SVG when there is no data', () => {
    const svg = element().querySelector('svg.m3k-donut-chart');
  ...
  it('renders the center value and label when provided', () => {
    ...
    expect(element().querySelector('.m3k-donut-chart__center-value')?.textContent?.trim()).toBe(
      '110',
    );
    expect(element().querySelector('.m3k-donut-chart__center-label')?.textContent?.trim()).toBe(
      'Invoices',
    );
  });
  ...
  it('hides center text by default', () => {
    ...
    expect(element().querySelector('.m3k-donut-chart__center-value')).toBeNull();
    expect(element().querySelector('.m3k-donut-chart__center-label')).toBeNull();
  });
