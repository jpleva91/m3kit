...
describe('BarChartComponent', () => {
  ...
  const bars = (): readonly Element[] => [...element().querySelectorAll('.m3k-bar-chart__bar')];
  ...
  it('renders an accessible empty SVG when there is no data', () => {
    const svg = element().querySelector('svg.m3k-bar-chart');
  ...
  it('renders nice value ticks and centered category labels', () => {
    ...
    expect(tickLabels('.m3k-bar-chart__tick--value')).toEqual(['0', '2', '4', '6', '8', '10']);
    expect(tickLabels('.m3k-bar-chart__tick--category')).toEqual(['Q1', 'Q2']);
    const categoryTicks = [...element().querySelectorAll('.m3k-bar-chart__tick--category')];
    expect(categoryTicks.map((el) => el.getAttribute('x'))).toEqual(['180', '452']);
    expect(element().querySelectorAll('.m3k-bar-chart__grid-line')).toHaveLength(6);
  });
  ...
  it('draws negative grouped values below the zero baseline', () => {
    ...
    expect(tickLabels('.m3k-bar-chart__tick--value')).toEqual(['-5', '0', '5', '10']);
  });
  ...
  it('swaps axes in horizontal mode', () => {
    ...
    const valueTicks = [...element().querySelectorAll('.m3k-bar-chart__tick--value')];
    ...
    const categoryTick = element().querySelector('.m3k-bar-chart__tick--category');
  ...
  it('adopts the measured host width as the viewBox width and rescales', () => {
    ...
    try {
      ...
      expect(root.querySelector('svg.m3k-bar-chart')?.getAttribute('viewBox')).toBe('0 0 320 240');
      // Plot 44..308 → band 132, inset 19.8, bar width 92.4; heights keep.
      const first = root.querySelector('.m3k-bar-chart__bar');
