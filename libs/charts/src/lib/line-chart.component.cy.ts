...
@Component({
  ...
  template: `
    <m3k-line-chart [series]="series" [area]="area" ariaLabel="Revenue and costs by month" />
  `,
})
...
describe(LineChartComponent.name, () => {
  it('renders one token-colored path per series', () => {
    mountLineChart();
    cy.get('svg.m3k-line-chart')
      .should('have.attr', 'role', 'img')
      .and('have.attr', 'aria-label', 'Revenue and costs by month');
    cy.get('.m3k-line-chart__line').should('have.length', 2);
    cy.get('.m3k-line-chart__line').eq(0).should('have.attr', 'stroke', 'var(--app-chart-1)');
    cy.get('.m3k-line-chart__line').eq(1).should('have.attr', 'stroke', 'var(--app-chart-2)');
  });

  it('renders axes, grid lines, and first/last x labels', () => {
    mountLineChart();
    cy.get('.m3k-line-chart__axis').should('have.length', 2);
    cy.get('.m3k-line-chart__grid-line').should('have.length.greaterThan', 2);
    cy.get('.m3k-line-chart__tick--x').first().should('contain.text', 'Jan');
    cy.get('.m3k-line-chart__tick--x').last().should('contain.text', 'Apr');
  });

  it('adds translucent area fills when area is on', () => {
    mountLineChart({ area: true });
    cy.get('.m3k-line-chart__area')
  ...
  it('renders an empty accessible SVG without data', () => {
    ...
    cy.get('svg.m3k-line-chart').should('have.attr', 'role', 'img');
    cy.get('.m3k-line-chart__line').should('not.exist');
  });

  it('sizes the viewBox to the measured host width (1 unit = 1 CSS px)', () => {
    mountLineChart();
    cy.get('svg.m3k-line-chart').should(($svg) => {
