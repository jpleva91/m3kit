...
describe('ChartLegendComponent', () => {
  ...
  it('renders an empty list for no items', () => {
    expect(element().querySelectorAll('.m3k-chart-legend__item')).toHaveLength(0);
  });
  ...
  it('renders one swatch-and-label row per item', () => {
    ...
    const labels = [...element().querySelectorAll('.m3k-chart-legend__label')].map((el) =>
      el.textContent?.trim(),
    );
    expect(labels).toEqual(['Paid', 'Overdue']);

    const swatches = [
      ...element().querySelectorAll<HTMLElement>('.m3k-chart-legend__swatch'),
    ];
  ...
  it('cycles color indices past the sixth token', () => {
    ...
    const swatch = element().querySelector<HTMLElement>('.m3k-chart-legend__swatch');
