...
import type { FilterState, DataPage, DataQuery, SortState } from './query';

/** Options for `InMemoryTableDataSource`. */
export interface InMemoryTableDataSourceOptions<T> {
  /**
...
/**
 * Synchronous, array-backed `TableDataSource` implementing the full
 ...
 */
export class InMemoryTableDataSource<T> implements TableDataSource<T> {
  private readonly rows: readonly T[];
  private readonly textSearchKeys?: readonly (keyof T & string)[];
  private readonly latencyMs: number;

  constructor(rows: readonly T[], options: InMemoryTableDataSourceOptions<T> = {}) {
    this.rows = [...rows];
   ...
   */
  fetch(query: DataQuery): Observable<DataPage<T>> {
  ...
  private applyTextFilter(rows: readonly T[], filter: FilterState): readonly T[] {
  ...
  private applyFieldFilters(rows: readonly T[], filter: FilterState): readonly T[] {
  ...
  private applySort(rows: readonly T[], sort: SortState | null): readonly T[] {
    if (!sort) {
      return rows;
    }
  ...
  private paginate(rows: readonly T[], query: DataQuery): DataPage<T> {
