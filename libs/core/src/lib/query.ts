...
/**
 * Complete query a report view sends to a `TableDataSource`:
...
/**
 * One page of report results, as returned by a `TableDataSource`.
 ...
 */
export function createDefaultQuery(pageSize: number = DEFAULT_PAGE_SIZE): DataQuery {
  return {
    filter: {},
    sort: null,
 ...
 */
export function createEmptyPage<T>(query: DataQuery = createDefaultQuery()): DataPage<T> {
  return {
    rows: [],
    totalCount: 0,
