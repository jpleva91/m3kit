...
describe(KpiStripComponent.name, () => {
  it('renders one readout per item with formatted values', () => {
    mountKpiStrip();
    cy.get('.m3k-kpi-strip__readout').should('have.length', 3);
    cy.get('.m3k-kpi-strip__label').first().should('have.text', 'Total revenue');
    cy.get('.m3k-kpi-strip__value').eq(0).should('have.text', '$1,284,902.44');
    cy.get('.m3k-kpi-strip__value').eq(1).should('have.text', '38');
    cy.get('.m3k-kpi-strip__value').eq(2).should('have.text', '92%');
  });

  it('renders a positive delta with an upward arrow and explicit sign', () => {
    mountKpiStrip();
    cy.get('.m3k-kpi-strip__readout')
      .eq(0)
      .find('.m3k-kpi-strip__delta')
      .should('have.class', 'm3k-kpi-strip__delta--up')
      .and('contain.text', '+4.2');
    cy.get('.m3k-kpi-strip__readout')
      .eq(0)
      .find('.m3k-kpi-strip__delta-icon')
  ...
  it('renders a negative delta with a downward arrow', () => {
    ...
    cy.get('.m3k-kpi-strip__readout')
      .eq(1)
      .find('.m3k-kpi-strip__delta')
      .should('have.class', 'm3k-kpi-strip__delta--down')
      .and('contain.text', '-6');
    cy.get('.m3k-kpi-strip__readout')
      .eq(1)
      .find('.m3k-kpi-strip__delta-icon')
  ...
  it('hides the delta entirely when omitted', () => {
    mountKpiStrip();
    cy.get('.m3k-kpi-strip__readout').eq(2).find('.m3k-kpi-strip__delta').should('not.exist');
  });
  ...
  it('draws the sparkline as an inline SVG polyline', () => {
    ...
    cy.get('.m3k-kpi-strip__readout')
      .eq(1)
      .find('svg.m3k-kpi-strip__sparkline polyline')
      .invoke('attr', 'points')
  ...
  it('omits the sparkline when fewer than two points are given', () => {
    ...
    cy.get('svg.m3k-kpi-strip__sparkline').should('not.exist');
  });
  ...
  it('formats currency with the configured currency code', () => {
    ...
    cy.get('.m3k-kpi-strip__value').should('have.text', '€1,000.00');
  });
});
