...
@Component({
  ...
  template: `<m3k-kpi-strip [items]="items" [currencyCode]="currencyCode" />`,
})
class HostComponent {
...
describe('KpiStripComponent', () => {
  ...
  const readouts = (): readonly HTMLElement[] =>
    Array.from(element().querySelectorAll<HTMLElement>('.m3k-kpi-strip__readout'));
  ...
  it('renders one readout per item with its label', () => {
    ...
    expect(readouts().map((readout) => text(readout, '.m3k-kpi-strip__label'))).toEqual([
      'Total revenue',
      'Open invoices',
      'Overdue',
      'Health',
  ...
  it('formats numeric values and passes strings through untouched', () => {
    const values = readouts().map((readout) => text(readout, '.m3k-kpi-strip__value'));
    expect(values).toEqual(['$1,284,902.44', '38', '12', 'nominal']);
  });

  it('renders positive deltas with an upward arrow and the up class', () => {
    const delta = readouts()[0].querySelector('.m3k-kpi-strip__delta');
    expect(delta?.classList).toContain('m3k-kpi-strip__delta--up');
  ...
  it('renders negative deltas with a downward arrow and the down class', () => {
    const delta = readouts()[2].querySelector('.m3k-kpi-strip__delta');
    expect(delta?.classList).toContain('m3k-kpi-strip__delta--down');
  ...
  it('hides the delta when omitted or null', () => {
    expect(readouts()[3].querySelector('.m3k-kpi-strip__delta')).toBeNull();

    host.items = [{ label: 'Overdue', value: 12, delta: null }];
    fixture.detectChanges();
    expect(readouts()[0].querySelector('.m3k-kpi-strip__delta')).toBeNull();
  });
  ...
  it('renders a sparkline polyline scaled into the viewBox', () => {
    ...
    const polyline = element().querySelector('.m3k-kpi-strip__sparkline polyline');
  ...
  it('labels the sparkline after the metric', () => {
    ...
    expect(
      element().querySelector('.m3k-kpi-strip__sparkline')?.getAttribute('aria-label'),
    ).toBe('Revenue trend');
  });

  it('hides the sparkline for missing or single-point series', () => {
    expect(element().querySelector('.m3k-kpi-strip__sparkline')).toBeNull();
