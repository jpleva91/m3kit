...
describe(DonutChartComponent.name, () => {
  it('renders one token-colored arc per slice', () => {
    mountDonutChart();
    cy.get('svg.rpt-donut-chart')
      .should('have.attr', 'role', 'img')
      .and('have.attr', 'aria-label', 'Invoices by status');
    cy.get('.rpt-donut-chart__slice').should('have.length', 3);
    cy.get('.rpt-donut-chart__slice[data-slice="Paid"]').should(
      'have.attr',
      'stroke',
      'var(--app-chart-1)',
    );
  ...
  it('shows the center value and label', () => {
    ...
    cy.get('.rpt-donut-chart__center-value').should('be.visible').and('contain.text', '142');
    cy.get('.rpt-donut-chart__center-label').should('be.visible').and('contain.text', 'Invoices');
  });

  it('hides center text when not provided', () => {
    mountDonutChart();
    cy.get('.rpt-donut-chart__center-value').should('not.exist');
    cy.get('.rpt-donut-chart__center-label').should('not.exist');
  });

  it('covers the full circle: slice percentages sum to 100', () => {
    mountDonutChart();
    cy.get('.rpt-donut-chart__slice').then(($slices) => {
