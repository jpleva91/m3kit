...
describe('LineChartComponent', () => {
  ...
  const lines = (): readonly Element[] => [...element().querySelectorAll('.rpt-line-chart__line')];
  ...
  it('renders an accessible empty SVG when there is no data', () => {
    const svg = element().querySelector('svg.rpt-line-chart');
  ...
  it('scales a bare (axis-free) chart into the slim-padded plot exactly', () => {
    ...
    expect(element().querySelector('.rpt-line-chart__grid-line')).toBeNull();
    expect(element().querySelector('.rpt-line-chart__axis')).toBeNull();
  });
  ...
  it('closes the area path down to the domain baseline', () => {
    ...
    const area = element().querySelector('.rpt-line-chart__area');
  ...
  it('renders no area fill by default', () => {
    ...
    fixture.detectChanges();
    expect(element().querySelector('.rpt-line-chart__area')).toBeNull();
  });
  ...
  it('marks a single-point series with a token-colored circle', () => {
    ...
    const point = element().querySelector('.rpt-line-chart__point');
  ...
  it('renders no point marker for multi-point series', () => {
    ...
    expect(element().querySelector('.rpt-line-chart__point')).toBeNull();
  });
  ...
  it('renders nice y ticks, grid lines, and category x labels', () => {
    ...
    expect(tickLabels('.rpt-line-chart__tick--y')).toEqual(['0', '2', '4', '6', '8', '10']);
    expect(tickLabels('.rpt-line-chart__tick--x')).toEqual(['Jan', 'Feb', 'Mar']);
    expect(element().querySelectorAll('.rpt-line-chart__grid-line')).toHaveLength(6);

    // Plot box: left 44, right 588, top 12, bottom 212 (height 240).
    expect(lines()[0].getAttribute('d')).toBe('M44,212 L316,112 L588,12');
    const yTickEls = [...element().querySelectorAll('.rpt-line-chart__tick--y')];
  ...
  it('labels only the first, last, and a few interior points of a long series', () => {
    ...
    expect(tickLabels('.rpt-line-chart__tick--x')).toEqual(['D0', 'D3', 'D6', 'D8', 'D11']);
  });
  ...
  it('adopts the measured host width as the viewBox width and rescales', () => {
    ...
    try {
      ...
      expect(root.querySelector('svg.rpt-line-chart')?.getAttribute('viewBox')).toBe(
        '0 0 320 104',
      );
      // Margins 4 → the line now ends at x = 320 - 4 = 316.
      expect(root.querySelector('.rpt-line-chart__line')?.getAttribute('d')).toBe(
        'M4,100 L316,4',
      );
    } finally {
