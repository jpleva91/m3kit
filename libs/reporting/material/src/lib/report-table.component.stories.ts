...
const PENDING_DATA_SOURCE: ReportDataSource<InvoiceRow> = {
  fetch: () => NEVER,
};

const meta: Meta<ReportTableComponent<InvoiceRow>> = {
  component: ReportTableComponent,
  title: 'Material/ReportTable',
  decorators: [applicationConfig({ providers: [provideAnimations()] })],
};
export default meta;
type Story = StoryObj<ReportTableComponent<InvoiceRow>>;

export const Invoices: Story = {
  args: {
    definition: INVOICE_DEFINITION,
    dataSource: new InMemoryReportDataSource(INVOICES),
  },
};
...
export const Empty: Story = {
  args: {
    definition: INVOICE_DEFINITION,
    dataSource: new InMemoryReportDataSource<InvoiceRow>([]),
  },
};
