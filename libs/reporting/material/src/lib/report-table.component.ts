 ...
 * to `rpt-report-filter-bar`'s `filterChange` output) or imperatively
...
export class ReportTableComponent<T> {
  ...
  readonly dataSource = input.required<ReportDataSource<T>>();
  ...
  private readonly sort = linkedSignal<ReportSortState<T> | null>(
  ...
  readonly query = computed<ReportQuery>(() => {
    ...
    return {
      ...
      // Widen `keyof T & string` to the untyped sort state `ReportQuery` carries.
  ...
  protected formatCell(row: T, column: ReportColumnDef<T>): string {
  ...
  protected badgeColor(row: T, column: ReportColumnDef<T>): string {
    const value = (row as Record<string, unknown>)[column.key];
