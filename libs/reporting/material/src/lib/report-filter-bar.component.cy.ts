...
} from './report-filter-bar.component';
...
class FilterBarHostComponent {
  ...
  readonly emissions = signal<readonly ReportFilterBarChange[]>([]);

  onFilterChange(change: ReportFilterBarChange): void {
    this.emissions.update((emissions) => [...emissions, change]);
  }
}

describe(ReportFilterBarComponent.name, () => {
  it('emits the typed text only after the debounce elapses', () => {
    ...
    cy.mount(FilterBarHostComponent, {
      ...
      cy.tick(REPORT_FILTER_DEBOUNCE_MS);
      cy.then(() => {
