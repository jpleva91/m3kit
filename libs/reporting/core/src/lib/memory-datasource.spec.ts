...
import { InMemoryReportDataSource } from './memory-datasource';
import { createDefaultQuery, ReportPage, ReportQuery } from './query';
...
function fetchPage(
  source: InMemoryReportDataSource<OrderRow>,
  q: ReportQuery
): Promise<ReportPage<OrderRow>> {
  return firstValueFrom(source.fetch(q));
}

describe('InMemoryReportDataSource', () => {
  let source: InMemoryReportDataSource<OrderRow>;

  beforeEach(() => {
    source = new InMemoryReportDataSource(ORDERS);
  });
  ...
  describe('no filter', () => {
    ...
    it('does not mutate or share the original rows array', async () => {
      const rows = [...ORDERS];
      const local = new InMemoryReportDataSource(rows);
  ...
  describe('text filter', () => {
    ...
    it('restricts the search to textSearchKeys and stringifies their values', async () => {
      const keyed = new InMemoryReportDataSource(ORDERS, { textSearchKeys: ['id', 'status'] });
  ...
  describe('latency and dataset edges', () => {
    ...
    it('delays emission when a latency option is supplied', async () => {
      const delayed = new InMemoryReportDataSource(ORDERS, { latencyMs: 20 });
    ...
    it('handles an empty dataset', async () => {
      const empty = new InMemoryReportDataSource<OrderRow>([]);
      const page = await fetchPage(
        empty,
