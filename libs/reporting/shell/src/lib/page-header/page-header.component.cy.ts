...
describe(PageHeaderComponent.name, () => {
  ...
  it('omits the subtitle when empty and renders it when provided', () => {
    mountPageHeader();
    cy.get('.rpt-page-header__subtitle').should('not.exist');

    mountPageHeader({ subtitle: 'Billing period June 2026' });
    cy.get('.rpt-page-header__subtitle').should(
      'have.text',
  ...
  it('aligns projected actions to the header end', () => {
    mountPageHeader();
    cy.get('.rpt-page-header__actions .probe-action').should('be.visible');
    cy.get('.rpt-page-header').then(($header) => {
