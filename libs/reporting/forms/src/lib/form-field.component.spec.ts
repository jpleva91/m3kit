...
describe('FormFieldComponent', () => {
  ...
  it('shows a wrapper error for a touched invalid checkbox', () => {
    ...
    expect(element.querySelector('.rpt-form-field__error')).toBeNull();

    host.control.markAsTouched();
    fixture.detectChanges();

    expect(element.querySelector('.rpt-form-field__error')?.textContent).toContain(
  ...
  it('renders a slide toggle that writes through the control', () => {
    ...
    expect(element.querySelector('.rpt-form-field__hint')?.textContent).toContain(
      'Adds closed records.',
    );
