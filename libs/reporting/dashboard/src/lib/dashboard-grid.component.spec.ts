...
@Component({
  ...
  template: `
    <rpt-dashboard-grid [minColumnWidth]="minColumnWidth" [gap]="gap">
      ...
      <div class="cell" [rptGridSpan]="span">B</div>
    </rpt-dashboard-grid>
  `,
})
class HostComponent {
...
describe('DashboardGridComponent', () => {
  ...
  const grid = (): HTMLElement =>
    element().querySelector('rpt-dashboard-grid') as HTMLElement;
  ...
  it('exposes the minimum column width as a CSS variable and applies the gap', () => {
    expect(grid().style.getPropertyValue('--rpt-dashboard-grid-min-column-width')).toBe('16rem');
    ...
    expect(grid().style.getPropertyValue('--rpt-dashboard-grid-min-column-width')).toBe('12rem');
    expect(grid().style.gap).toBe('2rem');
  });

  it('spans a child across columns via rptGridSpan', () => {
    const spanned = element().querySelectorAll<HTMLElement>('.cell')[1];
    expect(spanned.style.gridColumn).toBe('span 2');
  });

  it('stretches a child across the full row with rptGridSpan="full"', () => {
    host.span = 'full';
    fixture.detectChanges();
