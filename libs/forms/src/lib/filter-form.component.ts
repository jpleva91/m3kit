...
import { ColumnDef, ColumnType, TableDefinition } from '@m3kit/core';

import { FormFieldComponent, FormFieldOption, FormFieldType } from './form-field.component';

/** Filters emitted by `m3k-filter-form`: dirty, non-empty values by column key. */
...
/**
 * Field-filter form generated from a `TableDefinition<T>`: builds one
 ...
 *
 * Composable with `m3k-data-table`: wire `filtersChange` to the
 * table's `fieldFilters` input.
 */
@Component({
  selector: 'm3k-filter-form',
...
export class FilterFormComponent<T> {
  ...
  protected controlFor(key: string): FormControl<unknown> {
    ...
    if (!control) {
      throw new Error(`m3k-filter-form: no control for column "${key}".`);
    }
    return control;
  }

  /** Field type rendered for a column. */
  protected fieldTypeFor(column: ColumnDef<T>): FormFieldType {
...
const FIELD_TYPE_BY_COLUMN_TYPE: Readonly<Record<ColumnType, FormFieldType>> = {
  text: 'text',
  number: 'number',
  currency: 'currency',
  date: 'date',
...
function createFilterControl(type: ColumnType): FormControl<unknown> {
  switch (type) {
    case 'number':
    case 'currency':
