...
@Component({
  ...
  template: `
    <m3k-form-section [title]="title" [description]="description">
...
describe(FormSectionComponent.name, () => {
  it('renders the section heading', () => {
    mountFormSection();
    cy.get('.m3k-form-section__title')
  ...
  it('renders the description under the heading when provided', () => {
    mountFormSection();
    cy.get('.m3k-form-section__description').should('have.text', 'Where invoices are sent.');
  });

  it('omits the description paragraph when empty', () => {
    mountFormSection({ description: '' });
    cy.get('.m3k-form-section__description').should('not.exist');
  });

  it('projects fields into the content slot', () => {
    mountFormSection();
    cy.get('.m3k-form-section__content .projected-field').should('be.visible');
  });
});
