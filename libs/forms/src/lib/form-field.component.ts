...
/** Input control rendered by `m3k-form-field`. */
 ...
 *
 * ```html
 * <m3k-form-field label="Amount" type="currency" [control]="amount" />
 * <m3k-form-field label="Billing period" type="date-range" [range]="period" />
 * ```
 */
@Component({
  selector: 'm3k-form-field',
  imports: [
    MatAutocompleteModule,
...
export class FormFieldComponent<V = unknown> {
  ...
  protected ctrl(): FormControl<V> {
    ...
    if (!control) {
      throw new Error(
        `m3k-form-field: type "${this.type()}" requires the "control" input.`,
      );
    }
    return control;
  }
  ...
  protected rangeGroup(): DateRangeGroup {
    ...
    if (!range) {
      throw new Error('m3k-form-field: type "date-range" requires the "range" input.');
    }
    return range;
  }
